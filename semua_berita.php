...
require_once 'functions/function_berita_artikel.php';

$dataBerita = getAllBerita();
?>
...
<head>
  ...
  <title>Semua Berita & Artikel - Desa Cibening</title>
              ...
              <div class="col-md-4 mb-4">
                <div class="card h-100 shadow-sm">
                  <?php if (!empty($row['gambar'])): ?>
                    <img src="uploads/<?= htmlspecialchars($row['gambar']) ?>" class="card-img-top" style="height: 180px; object-fit: cover;" alt="Gambar Berita">
                  <?php endif; ?>
                  <div class="card-body">
                    <h5 class="card-title"><?= htmlspecialchars($row['judul']) ?></h5>
                    <p class="text-muted mb-1"><i class="bi bi-calendar-event"></i> <?= date('d M Y', strtotime($row['tanggal'])) ?></p>
                    <p class="card-text"><?= substr(strip_tags($row['isi']), 0, 100) ?>...</p>
                    <a href="detail_berita.php?id=<?= htmlspecialchars($row['id_berita']) ?>" class="btn btn-sm btn-baca mt-2">Baca Selengkapnya</a>
                  </div>
                </div>
              </div>
            <?php endforeach; ?>
